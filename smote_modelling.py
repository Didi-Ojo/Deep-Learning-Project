...
#Predictive Analysis using CNN-LSTM Hydrid model for Automobile Fraud Detection - WITH SMOTE
import numpy as np
import pandas as pd
...
def SMOTE_data_processing(filename):
...
def run_cnn_lstm_model(X_train, X_test, y_train, y_test):
    ...
    plt.legend()
    plt.savefig("Images/Accuracy_Epoch_SMOTE.jpg",transparent = False)
    plt.show()
    ...
    plt.legend()
    plt.savefig("Images/Loss_Epoch_SMOTE.jpg",transparent = False)
    plt.show()
    ...
    #Retraining the model using best epoch
    cnn_lstm_model = cnn_lstm((X_train.shape[1],X_train.shape[2]))
    ...
    #Evaluating the Model Accuracy on train and test set
    ...
    #Confusion matrix of the train and test set
    ...
    plt.savefig("Images/Confusion_Matrix_Train_SMOTE.jpg",transparent = False)
    plt.show()
    ...
    ax.xaxis.set_ticklabels(['Not Fraud','Fraud'])
    ax.yaxis.set_ticklabels(['Not Fraud','Fraud'])
    plt.savefig("Images/Confusion_Matrix_Test_SMOTE.jpg",transparent = False)
    plt.show()
...
def print_results(cnn_lstm_results,ML_results):
    ...
    plt.savefig("Images/Model_Comparison_SMOTE.jpg")
    plt.rcParams['figure.figsize'] = [13, 5]
    plt.show()
    

#DRIVER CODE
X_train, X_test, y_train, y_test = SMOTE_data_processing("Insurance_Data.csv")
